...
public class IFChargedTNTBock extends Block implements IForgeBlock {
	...
	public void catchFire(BlockState state, World world, BlockPos pos, @Nullable Direction face,
		...
		if (!world.isClientSide) {
			IFChargedTNTEntity chargedtntentity = new IFChargedTNTEntity(world, (double) pos.getX() + 0.5D,
					(double) pos.getY(), (double) pos.getZ() + 0.5D, igniter);
			world.addFreshEntity(chargedtntentity);
			world.playSound((PlayerEntity) null, chargedtntentity.getX(), chargedtntentity.getY(),
					chargedtntentity.getZ(), SoundEvents.TNT_PRIMED, SoundCategory.BLOCKS, 1.0F, 1.0F);
		}
	}
	...
	public void wasExploded(World worldIn, BlockPos pos, Explosion explosionIn) {
		if (!worldIn.isClientSide) {
			TNTEntity tntentity = new TNTEntity(worldIn, (double) pos.getX() + 0.5D,
